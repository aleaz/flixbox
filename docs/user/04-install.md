...
> **Implementation status:** Full Compose stack + `bin/flixbox` CLI are **Implemented**. Optional profiles: `plex`, `proxy`, `recyclarr`, `notifications`, `vpn-heal`. `docker-socket-proxy` always runs with Homepage.
