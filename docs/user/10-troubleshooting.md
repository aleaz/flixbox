...
| Homepage looks old after `git pull` / `reload` (missing dots, CSS, logo) | Live `${CONFIG_DIR}/homepage` is copy-if-missing; templates in the repo do not overwrite | `./bin/flixbox homepage refresh` (backup + apply + restart) or `./bin/flixbox reload --reset-homepage` — [REFERENCE](REFERENCE.md) |
