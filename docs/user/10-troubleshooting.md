...
| Radarr/Sonarr qBit **Test** fails (auth) | Stale password after qBit recreate, or wrong credential type | `docker compose restart qbittorrent` (clears ban), then `./bin/flixbox configure` (re-tests and updates the download client). Prefer qBit **API key** in *arr (username/password optional). — [Credentials](06-configuration.md#credentials-and-api-keys) |
...
| Decluttarr / *arr `403` / “IP has been banned” | Failed logins before Docker subnet whitelist, or stale *arr password after qBit recreate | `docker compose restart qbittorrent` (clears in-memory ban) → `./bin/flixbox configure` → confirm Decluttarr logs `OK \| qBittorrent`. Whitelist is `172.30.42.0/24` only — [ADR 0008](../adr/0008-maintenance-decluttarr-maintainerr.md) |
