...
| Changed qBit password (or API key) in the WebUI only | `.env` / Decluttarr / *arr still have old values | Align `.env` with the WebUI password if you changed it there, then `./bin/flixbox configure --sync-qbit-auth` — [Credentials](06-configuration.md#qbit-password--api-key-changes-source-of-truth) |
