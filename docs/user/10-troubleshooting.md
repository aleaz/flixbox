...
| Seerr restart-loop / `unhealthy` on first start | Config dir not writable by UID **1000** (Seerr ignores `PUID`) | `./bin/flixbox init --non-interactive` (sets ownership; fails closed if it cannot). Manual: `docker run --rm -v "${CONFIG_DIR}/seerr:/data" alpine chown -R 1000:1000 /data` then recreate Seerr |
...
| Homepage: `Host validation failed` from a phone/LAN PC | `HOMEPAGE_ALLOWED_HOSTS` only lists localhost | Set `FLIXBOX_PUBLIC_HOST` then `./bin/flixbox reload` (auto-extends allowlist) — or add `LAN_IP:3000` by hand — [§13 Homepage links](13-access-profiles.md#homepage-links-from-phones--tvs) |
| Homepage Jellyfin/Seerr links open `localhost` on another device | `FLIXBOX_PUBLIC_HOST` unset | Set `FLIXBOX_PUBLIC_HOST`, then `./bin/flixbox reload` — [§13](13-access-profiles.md#homepage-links-from-phones--tvs) |
| Homepage admin links under `shared` | On host: `http://127.0.0.1:<port>` + Forms. On LAN: ports not published | Open Homepage on the server (or SSH tunnel); `credentials set arr-ui` — [§13](13-access-profiles.md) |
| Homepage: `Failed to load services.yaml` after switching to `shared` | `homepage-sync` used to stop skipping admin widgets at nested `- level:` / `- field:` lines, leaving orphan YAML | Fixed in `scripts/lib/homepage-sync.py`. Repair: `cp templates/homepage/services.yaml "${CONFIG_DIR}/homepage/services.yaml"` then `./bin/flixbox reload` (or `homepage-sync` via `up`/`configure`). Backup the broken file first. |
...
| Jellyfin: “No compatible streams” / “Could not find a valid media source” (UI loads, play fails) | `network.xml` **Bind to local network address** set to `::` (IPv6 any) — Jellyfin advertises `::1` stream URLs; breaks on localhost **and** LAN | Re-run `./bin/flixbox configure` (clears a lone `::` bind and restarts Jellyfin). Manual: Dashboard → Networking → clear bind (leave empty); optional Published Server URL `http://<host>:8096`. Clearing bind does **not** change LAN exposure — Compose still publishes `:8096` ([§13](13-access-profiles.md)). |
| Jellyfin external `.srt` missing in player | Library not rescanned after Bazarr download, or preferred subtitle language unset | Refresh/scan the item; set preferred subtitle language on the user (or library). Filename must share the video basename (`video.es-MX.srt` / `video.spa.srt`). |
...
| `./bin/flixbox configure` fails on first run (API not ready) | *arr/Jellyfin still initializing SQLite | Waits up to **15 min** (`CONFIGURE_PREFLIGHT_TIMEOUT=900`); phases capped by remaining budget — typical 1–3 min — [First-run §0](05-first-run.md#0-script-assisted-wiring-recommended) |
...
**Homepage / socket-proxy:** Compose mounts `/var/run/docker.sock` into `docker-socket-proxy` only (Homepage uses `docker-socket-proxy:2375` on `flixbox_net`). First-class path is rootful Docker Engine. Rootless Docker or a custom `DOCKER_HOST` may require adapting that mount — see [ADR 0007](../adr/0007-platform-support-tiers.md) and [ADR 0022](../adr/0022-operator-footgun-remediations.md).
