...
- Support **dual mode** via `VPN_ENABLED`:
  - `true`: qBittorrent uses `network_mode: service:gluetun` with killswitch + DoT; publish qBit ports on **Gluetun**; client host `gluetun`.
  - `false`: qBittorrent on `flixbox_net`; client host `qbittorrent`.
...
- Compose must ship two downloader modules (or equivalent profiles).
