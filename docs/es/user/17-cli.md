...
Con `--json`, stdout es un solo objeto `{ schemaVersion, ok, dryRun, summary }` (sin secretos).
...
`-q`/`-v` globales, `--env-file`, `--project-dir`; migración completa `die`→taxonomía (Phase C); completions fish; digest pins — ver [08-roadmap.md](../../08-roadmap.md).
